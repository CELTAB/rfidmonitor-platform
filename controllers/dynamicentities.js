...
DynamicEntities.prototype.typesEnumToPostgres = function(type){
	switch(type){
		case DynamicEntities.prototype.typesEnum.RFIDCODE :
			return 'TEXT';
			break;
		case DynamicEntities.prototype.typesEnum.TEXT:
			return 'TEXT';
			break;
		case DynamicEntities.prototype.typesEnum.NUMBER:
			return 'INTEGER';
			break;
		case DynamicEntities.prototype.typesEnum.IMAGE:
			return 'TEXT';
			break;
...
var validateEntityField = function(field){
	...
	if(field["type"] == DynamicEntities.prototype.typesEnum.ENTITY){
		...
		if(!field["unique"]){
			errors.push({obj : field.field, error : "unique not found"});
		} else if(!isArray(field["unique"])){
			errors.push({obj : field.field, error : "unique is not array"});
		}else{
			//TODO validate the unique field names. they should exist.
		}
...
DynamicEntities.prototype.validateEntityCreateObject = function(json, callback){
	...
	structureList should have _status field and SHOULD NOT have a meta obj.
	shouldnt have 2 fields with same name in the same hierachy.
	unique fields should contain false, or a field name that exists in the structure.
	the deep level allowed is 2.
...
//TODO remove from public.
DynamicEntities.prototype.createMeta = function(json, callback){
...
var createTableEntity = function(entityObj) {
	...
	for(var iatt in entityObj.structureList){
		var attr = entityObj.structureList[iatt];
		...
		if(attr.type == DynamicEntities.prototype.typesEnum.ENTITY){
			...
			foreignQuery = foreignQuery.replace('$constraintName', 'fk_'+entityObj.meta.identifier + '_' + attr.meta.identifier);
			foreignQuery = foreignQuery.replace('$localField', attr.meta.identifier + '_id');
			foreignQuery = foreignQuery.replace('$foreignTable', attr.meta.dbTableName);
			foreignQuery = foreignQuery.replace('$foreignFieldId', attr.meta.dbPk);

			finalConstraints += foreignQuery;

			createTableEntity(attr);
	...
	*/
	...
	if(entityObj.unique.length > 0){

		//For each unique array for this entity.
		for(var iuq in entityObj.unique){
			var uq = {};
			uq.name = "uq_"+ entityObj.meta.identifier;
			uq.fieldStr = '';

			//for each unique field  for a specific contraint
			var uqFieldslength = entityObj.unique[iuq].length;

			for (var ia in entityObj.unique[iuq]){
				var uqFieldName = entityObj.unique[iuq][ia];
				uq.name += "_" + uqFieldName;

				if(ia == (uqFieldslength-1)){
					uq.fieldStr += uqFieldName;
				}else{
					uq.fieldStr += uqFieldName + ', ';
				}
			}
			finalConstraints += ", CONSTRAINT " +uq.name+ " UNIQUE ("+uq.fieldStr+")";
		}
	}

	finalConstraints +=  ", CONSTRAINT pk_" +entityObj.meta.identifier+ " PRIMARY KEY ("+entityObj.meta.dbPk+")";
...
//TODO remove from public.
DynamicEntities.prototype.buildPostgresDb = function(metaObj, callback){
	/*
		find deepest entities;
		build a list containing the order of creation.
		create the deepest tables;
		each new object that has foreing relation, create the relation. At this point the required tables should be available.
	*/

	for(var ia in metaObj){
		var rootEntity = metaObj[ia];
		createTableEntity(rootEntity);		
	}
...
//TODO remove from public.
DynamicEntities.prototype.buildDatabase = function(metaObj, callback){

	//if(POSTGRES)
	DynamicEntities.prototype.buildPostgresDb(metaObj, function(err, result){
		callback(err, result);
	});
}
...
DynamicEntities.prototype.createEntity = function(json, callback){
	...
	DynamicEntities.prototype.validateEntityCreateObject(json, function(err){
		//TODO. what about isValid variable? we are not using it.

		if(err)
			return callback(err, null);

		DynamicEntities.prototype.createMeta(json, function(err, metaObj){
			if(err)
				return callback(err, null);

			console.log(JSON.stringify(metaObj, null, '\t'));
			
			DynamicEntities.prototype.buildDatabase(metaObj, function(err, metaObj){
				if(err)
					return callback(err, null);
